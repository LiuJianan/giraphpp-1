...
public abstract class BasicRPCCommunications<I extends WritableComparable,
  ...
  // below are added for Giraph++
  /** Defines if the senderside Combiner should be used [needed in Giraph++] */
  private final VertexCombiner<I, M> sendersideCombiner;
  private boolean useSendersideCombiner = true;
    ...
    public void run() {
      ...
      try {
        ...
        synchronized (peerConnection.outMessagesPerPeer) {
          ...
          //## added for collecting stats
          int networkMsgs=0;
          int msgsBeforeCombine=0;
          ...
          for (Entry<I, MsgList<M>> entry :
            ...
            msgsBeforeCombine+=entry.getValue().size();
       
            if (sendersideCombiner != null && entry.getValue().size() > 1) {
              Iterable<M> messages = sendersideCombiner.combine(
            ...
            networkMsgs+=entry.getValue().size();
          }
          
          networkMessagesCounter.increment(networkMsgs);
          context.getCounter(GiraphJob.MSG_COUNTER_GROUP, "Network: Superstep "+service.getSuperstep()).increment(networkMsgs);
          messagedBeforeCombinerCounter.increment(msgsBeforeCombine);
          cached_context.getCounter(GiraphJob.MSG_COUNTER_GROUP, "Before Combine: Superstep "+service.getSuperstep()).increment(msgsBeforeCombine);
  ...
  private class LargeMessageFlushExecutor implements Runnable {
    ...
    public void run() {
      try {
        ...
        messagedBeforeCombinerCounter.increment(outMessageList.size());
        cached_context.getCounter(GiraphJob.MSG_COUNTER_GROUP, "Before Combine: Superstep "+service.getSuperstep()).increment(outMessageList.size());

        if (sendersideCombiner != null) {
          Iterable<M> messages = sendersideCombiner.combine(destVertex,
              outMessageList);
          if (messages == null) {
            throw new IllegalStateException(
                "run: Combiner cannot return null");
          ...
          networkMessagesCounter.increment(sentMsgs);
          cached_context.getCounter(GiraphJob.MSG_COUNTER_GROUP, "Network: Superstep "+service.getSuperstep()).increment(sentMsgs);
        } else {
          proxy.putMsgList(destVertex, outMessageList);
          networkMessagesCounter.increment(outMessageList.size());
          cached_context.getCounter(GiraphJob.MSG_COUNTER_GROUP, "Network: Superstep "+service.getSuperstep()).increment(outMessageList.size());
  ...
  public BasicRPCCommunications(Mapper<?, ?, ?, ?>.Context context,
    ...
    if (BspUtils.getVertexCombinerClass(conf) == null) {
      ...
      this.sendersideCombiner = this.service.getGraphMapper().getSenderSideCombiner();
    }
    
    networkMessagesCounter= context.getCounter(
    		GiraphJob.MSG_COUNTER_GROUP, "Network: Total");
    messagedBeforeCombinerCounter= context.getCounter(
    		GiraphJob.MSG_COUNTER_GROUP, "Before Combine: Total");
