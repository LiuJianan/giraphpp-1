...
public abstract class BasicVertex<I extends WritableComparable,
   ...
   */
  //changed for the hybrid model
  public void sendMsg(I id, M msg) {
    if (msg == null) {
      throw new IllegalArgumentException(
          "sendMsg: Cannot send null message to " + id);
    }
    GraphState<I,V,E,M> graphstate=getGraphState();
    Partition<I,V,E,M> part=graphstate.getCurrentPartition();
    if(part==null)
    {
    	graphstate.getWorkerCommunications().sendMessageReq(id, msg);
    }
    else
    {
    	if(part.getVertex(id)==null)
    	{
    		graphstate.getWorkerCommunications().sendMessageReq(id, msg);
    	}
    	else
    	{
    		part.addInternalMessage(id, msg);
    	}
    }
  }
