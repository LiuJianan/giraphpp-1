...
public class Partition<I extends WritableComparable,
  ...
  private VertexCombiner<I, M> combiner=null;
  private final Map<I, List<M>> internalMsgs=new HashMap<I, List<M>>();
  public void addInternalMessage(I vertex, M message)
  {
	  List<M> msglist=internalMsgs.get(vertex);
	  if(msglist==null)
	  {
		  msglist=new ArrayList<M>(2);
		  internalMsgs.put(vertex, msglist);
	  }
	  msglist.add(message);
	  if (combiner != null && msglist.size()>1) {
          try {
        	Iterator<M> iter = combiner.combine(vertex, msglist).iterator();
            ...
            throw new IllegalStateException(
                "addMessage: Combiner failed to combine messages " + msglist, e);
          }

        }
	  
  }
  public void setCombiner(VertexCombiner<I, M> comb)
  {
	  combiner=comb;
  }
  public List<M> getInternalMessages(I vertex)
  {
	  return internalMsgs.get(vertex);
  }
  
  public String printInternalMessages()
  {
	  return internalMsgs.toString();
  }
  
  public Map<I, BasicVertex<I, V, E, M>> getVerticesMap()
  {
	  return vertexMap;
  }
