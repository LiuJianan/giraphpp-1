...
public class SendMessageCache<I extends WritableComparable,
  ...
  public int getNumMessagesInCache()
  {
	  int n=0;
	  for(Integer i: messageCountMap.values())
		  n+=i;
	  return n;
  }
  ...
  public int addMessage(Integer partitionId, I destVertexId, M message) {
    ...
    if (combiner != null && messages.size()>1) {
          try {
        	Iterator<M> iter = combiner.combine(destVertexId, messages).iterator();
        	messages.clear();
        	while(iter.hasNext())
